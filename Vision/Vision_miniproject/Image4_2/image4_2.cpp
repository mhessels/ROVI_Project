...
cv::Mat BBSF(double n, int wy, int wx, int cx, int cy){
  ...
  double d0 = std::sqrt( double((2103-cx)*(2103-cx)) + double((1815-cy)*(1815-cy)));
  std::cout << d0;
  for(int y = 0; y < wy; ++y) {
	for(int x = 0; x < wx; ++x) {
	  ...
	  if(d_pos==0) // Avoid division by zero
		...
		nf(y,x)[0] = (1.0 / (1.0 + std::pow(d_pos*50/(std::pow(d_pos,2)-std::pow(d0,2)), 2.0*n)));
	}
  }
  return nf;
}
...
int main()
{
  ...
  // A gray image
  cv::Mat_<float> img = cv::imread("Image4_2.png", CV_LOAD_IMAGE_GRAYSCALE);
  // Load image
  ...
  cv::mulSpectrums(bsf, img_dft, img_dft, cv::DFT_ROWS);
  dftshift(img_dft);
  
  //Display high pass filter
  cv::Mat realImg[2];
  cv::split(bsf,realImg);
  cv::Mat realBSF = realImg[0];
  cv::normalize(realBSF, realBSF, 0.0, 1.0, CV_MINMAX);
  // 	namedWindow("", cv::WINDOW_NORMAL);
  cv::imwrite("BSF.png", realBSF);
